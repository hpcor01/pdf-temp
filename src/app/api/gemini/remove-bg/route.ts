...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "API Route: removeImageBackground result length:",
      result ? result.length : "null"
    );
    console.log(
      "API Route: removeImageBackground result starts with:",
    ...
    if (!result.startsWith("data:")) {
      console.error(
        "API Route: Result is not a valid data URL:",
        result.substring(0, 100)
      );
      return NextResponse.json(
        { error: "Invalid image format returned" },
        { status: 500 }
      );
    }
